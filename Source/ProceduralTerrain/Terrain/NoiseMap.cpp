...
TArray<TArray<float>> NoiseMap::GenerateNoiseMap(int mapWidth, int mapHeight, FNoiseSettings settings, FVector2D sampleCenter, bool isBlock)
{
	...
	for (int i = 0; i < settings.Octaves; i++) {
		...
		if (isBlock == true)
			offsetY = prng->FRandRange(-100000, 100000) + settings.Offset.Y + sampleCenter.Y;
		else
			offsetY = prng->FRandRange(-100000, 100000) - settings.Offset.Y - sampleCenter.Y;
	...
	for (int y = 0; y < mapHeight; y++) {
		for (int x = 0; x < mapWidth; x++) {
			...
			float normilizedHeight = (noiseMap[x][y] + 2.f) / (maxPossibleHeight / 0.9f);
...
void NoiseMap::Set2DArrayNum(TArray<TArray<float>>& array, int width, int height)
{
	array.SetNum(width);
...
void NoiseMap::Set2DArrayNum(TArray<TArray<int>>& array, int width, int height)
{
	array.SetNum(width);
